"""Mesh-to-depth alignment using one-way chamfer losses (no explicit correspondences).

Overview
--------
This script aligns human/object meshes to frame_00 depth in camera coordinates
with one-way chamfer terms in 3D and 2D:
...
Unlike fixed-correspondence alignment, this objective does not bind each observed
point to a precomputed mesh pixel. This is more robust when initial mesh
projections are offset.
"""
...
def save_chamfer_debug_snapshot(
    ...
    model3d: np.ndarray,
    model3d_colors: np.ndarray,
    model2d: np.ndarray,
    model2d_nn_colors_rgb: np.ndarray,
    nn2d_idx: np.ndarray,
    nn2d_d2: np.ndarray,
    nn3d_d2: np.ndarray,
    point_radius: int,
    ...
    save_colored_point_cloud(model3d_path, model3d, model3d_colors)
    ...
    draw_points_per_point_color(
        canvas_bgr=right,
        uv=model2d,
        colors_rgb=model2d_nn_colors_rgb,
        radius=point_radius,
        max_points=max_points_vis,
    )
    ...
    cv2.putText(
        right,
        "Projected transformed model samples (M2D', NN-colored)",
        (12, 28),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (255, 255, 255),
...
def save_loss_history_csv_chamfer(
    ...
    np.savetxt(
        ...
        delimiter=",",
        header="iter,total,cd3d,cd3d_raw,cd2d,scale_reg,t_reg,scale,tx,ty,tz",
        comments="",
    )
...
def compute_losses_chamfer_torch(
    model_points_base: torch.Tensor,
    obs3d: torch.Tensor,
    obs2d: torch.Tensor,
    ...
    model3d = scale * model_points_base
    model3d = model3d + torch.stack([tx, ty, tz]).view(1, 3)

    z = torch.clamp(model3d[:, 2], min=1e-6)
    u = intrinsics[0, 0] * model3d[:, 0] / z + intrinsics[0, 2] - 0.5
    v = intrinsics[1, 1] * model3d[:, 1] / z + intrinsics[1, 2] - 0.5
    model2d = torch.stack([u, v], dim=1)

    zero = torch.zeros((), device=model_points_base.device, dtype=model_points_base.dtype)

    if obs3d.shape[0] > 0:
        d2_3d, _ = min_distances_obs_to_model_chunked(
            obs=obs3d,
            model=model3d,
        ...
        cd3d_raw = d2_3d.mean()
        cd3d = geman_mcclure_func(cd3d_raw, rho=float(rho_geman_3d))
    else:
        cd3d_raw = zero
        cd3d = zero

    if obs2d.shape[0] > 0:
        d2_2d, _ = min_distances_obs_to_model_chunked(
            obs=obs2d,
            model=model2d,
            chunk_size=nn_chunk_size,
            return_indices=False,
...
def optimize_scale_txyz_chamfer(
    ...
    model_base_t = torch.from_numpy(model_points_base_np).to(
    ...
    def _save_debug(iter_idx: int, losses_eval: dict[str, torch.Tensor], save_fixed: bool) -> None:
        with torch.no_grad():
            model3d_t = losses_eval["model3d"]
            model2d_t = losses_eval["model2d"]
            model3d_np = model3d_t.detach().cpu().numpy()
            ...
            if (
                model_to_obs_idx_t is not None
                and obs3d_anchor_colors.shape[0] > 0
                and model3d_np.shape[0] == int(model_to_obs_idx_t.shape[0])
            ):
                model_to_obs_idx = model_to_obs_idx_t.detach().cpu().numpy().astype(np.int64)
                valid_idx = (
                    (model_to_obs_idx >= 0)
                    & (model_to_obs_idx < obs3d_anchor_colors.shape[0])
                )
                model3d_colors = np.zeros((model3d_np.shape[0], 3), dtype=np.uint8)
                if np.any(valid_idx):
                    model3d_colors[valid_idx] = obs3d_anchor_colors[model_to_obs_idx[valid_idx]]
                if np.any(~valid_idx):
                    model3d_colors[~valid_idx] = colorize_points_by_xyz(model3d_np[~valid_idx])
            else:
                model3d_colors = colorize_points_by_xyz(model3d_np)
            ...
            if model2d_to_obs2d_idx_t is not None and obs2d_anchor_colors.shape[0] > 0:
                ...
                model2d_nn_colors_rgb = np.zeros(
                    (model2d_t.shape[0], 3), dtype=np.uint8
                )
                valid_model2d_idx = (
                ...
                if np.any(valid_model2d_idx):
                    model2d_nn_colors_rgb[valid_model2d_idx] = obs2d_anchor_colors[
                        model2d_to_obs2d_idx[valid_model2d_idx]
                    ]
                if np.any(~valid_model2d_idx):
                    model2d_nn_colors_rgb[~valid_model2d_idx] = np.array(
                        [255, 140, 0], dtype=np.uint8
                    )
            else:
                model2d_nn_colors_rgb = np.tile(
                    np.array([255, 140, 0], dtype=np.uint8)[None, :],
                    (model2d_t.shape[0], 1),
                )
            ...
            save_chamfer_debug_snapshot(
                ...
                model3d=model3d_np,
                model3d_colors=model3d_colors,
                model2d=model2d_t.detach().cpu().numpy(),
                model2d_nn_colors_rgb=model2d_nn_colors_rgb,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Mesh-depth alignment with one-way 3D/2D chamfer losses and "
...
def main() -> None:
    ...
    summary_out = {
        ...
        "energy_terms": {
            ...
            "model3d": "M3D = sampled mesh surface points",
            "model2d": "M2D' = project(M3D')",
            "L_cd3d_raw": "mean_{o in O3D} min_{m in M3D'} ||o-m||_2^2",
            "L_cd3d": "GemanMcClure(L_cd3d_raw)",
            "L_cd2d": "mean_{o in O2D} min_{m in M2D'} ||o-m||_2^2",
            "L_scale_reg": "alpha^2",
