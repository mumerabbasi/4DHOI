...
4. Keep correspondences fixed and optimize global mesh scale + XYZ translation.
...
D) Optimization variables and transform
  ...
  Initialization:
    - `alpha = 0` -> `s = 1`
    - `t_x_init = t_y_init = t_z_init = 0`
    - `t = [t_x, t_y, t_z]^T = t_init + delta_t`

  Apply transform to the same fixed mesh correspondence points every iteration:
    `m_i'(alpha, delta_t) = s * m_i + t`
...
E) Losses (exact energy terms)
  ...
  Terms:
    ...
    - `E_scale_reg = alpha^2`
    - `E_t_reg     = delta_t_x^2 + delta_t_y^2 + delta_t_z^2`
...
F) What is saved
  ...
  - `overlay_after.png`: projection after optimized scale + t_xyz.
...
def new_history_dict_txyz() -> dict[str, list[float]]:
    return {
        "iter": [],
        "total": [],
        "corr": [],
...
def append_history_txyz(
    ...
    history["tz_reg"].append(float(losses["t_reg"].detach().cpu().item()))
...
def save_loss_history_csv_txyz(history: dict[str, list[float]], out_path: Path) -> None:
    ...
    np.savetxt(
        ...
        delimiter=",",
        header="iter,total,corr,reproj,scale_reg,t_reg,scale,tx,ty,tz",
        comments="",
    )


def compute_losses_txyz_torch(
    ...
    transformed = transformed + torch.stack([tx, ty, tz]).view(1, 3)
    ...
    e_t_reg = delta_tx.pow(2) + delta_ty.pow(2) + delta_tz.pow(2)
    e_total = (
        float(w_corr) * e_corr
        + float(w_reproj) * e_reproj
        + float(w_scale_reg) * e_scale_reg
        + float(w_t_reg) * e_t_reg
    )
    return {
        "total": e_total,
        "corr": e_corr,
...
def optimize_scale_txyz(
...
) -> OptimizationResultTxyz:
    n = int(corr.mesh_points_base.shape[0])
    if n == 0:
        return OptimizationResultTxyz(
    ...
    # Start from the mesh's current 3D placement: scale=1 and no extra translation.
    tx_init_val = 0.0
    ty_init_val = 0.0
    tz_init_val = 0.0
    ...
    with torch.no_grad():
        init_losses = compute_losses_txyz_torch(
        ...
        append_history_txyz(history, 0, init_losses)
        init_scale = float(init_losses["scale"].detach().cpu().item())
        init_full_mesh_points = (
            init_scale * full_mesh_points_base
        ).astype(np.float32)
        init_full_mesh_points[:, 0] += float(init_losses["tx"].detach().cpu().item())
        init_full_mesh_points[:, 1] += float(init_losses["ty"].detach().cpu().item())
    ...
    for iter_idx in range(1, int(iters) + 1):
        ...
        losses = compute_losses_txyz_torch(
            mesh_points_base=mesh_points_t,
            depth_points=depth_points_t,
            uv_ref=uv_ref_t,
            intrinsics=k_t,
        ...
        with torch.no_grad():
            ...
            eval_losses = compute_losses_txyz_torch(
                mesh_points_base=mesh_points_t,
                depth_points=depth_points_t,
                uv_ref=uv_ref_t,
                intrinsics=k_t,
            ...
            append_history_txyz(history, iter_idx, eval_losses)
            ...
            if (
                ...
                eval_full_mesh_points[:, 0] += float(
                    eval_losses["tx"].detach().cpu().item()
                )
                eval_full_mesh_points[:, 1] += float(
                    eval_losses["ty"].detach().cpu().item()
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="./output_scale_txyz_corr",
...
def main() -> None:
    ...
    optimization_results: list[OptimizationResultTxyz] = []
    ...
    for asset, corr, verts_cv in zip(assets, correspondences, verts_base_cv_np):
        ...
        print(f"[{asset.name}] optimizing scale + t_xyz ...")
        result = optimize_scale_txyz(
            corr=corr,
    ...
    summary_out = {
        ...
        "energy_terms": {
            "transform": (
                "m_i' = exp(alpha) * m_i + "
                "[t_x_init + delta_t_x, t_y_init + delta_t_y, "
                "t_z_init + delta_t_z]^T"
            ),
            ...
            "E_total": (
                ...
                "w_scale_reg*E_scale_reg + w_t_reg*E_t_reg"
            ),
        },
