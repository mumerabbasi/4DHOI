"""First-frame multi-seed object meshes with per-seed canonical meshes.

Each seed generates its own canonical mesh and posed mesh from the first frame.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Generate N first-frame mesh variants from multiple seeds. "
            "Each seed has its own canonical mesh."
        )
    )
    parser.add_argument(
        "--input_dir",
    ...
    parser.add_argument("--seed_stride", type=int, default=1, help="Step between seeds.")
    parser.add_argument(
        "--focal_length",
        type=float,
        default=None,
