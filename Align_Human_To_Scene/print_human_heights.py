...
def load_initial_vertices_camera(
    result_dir: Path,
    smplx_layer: Any,
    param_key: str,
    device: torch.device,
    ...
    with torch.no_grad():
        out = smplx_layer(
            transl=params["transl"][0].view(1, 3).to(device).float(),
            global_orient=params["global_orient"][0].view(1, 3).to(device).float(),
            body_pose=params["body_pose"][0].view(1, -1).to(device).float(),
            betas=params["betas"][0].view(1, -1).to(device).float(),
        )
...
def main() -> None:
    ...
    for track in tracks:
        init_verts_camera = load_initial_vertices_camera(
        ...
        print(f"\ntrack: {track.name}")
        print(
            f"  initial_world_height_m:   {init_height:.4f} "
            f"(z_min={init_z_min:.4f}, z_max={init_z_max:.4f})"
        )
        print(
            f"  optimized_world_height_m: {opt_height:.4f} "
            f"(z_min={opt_z_min:.4f}, z_max={opt_z_max:.4f})"
        )
        print(f"  delta_m:                  {opt_height - init_height:+.4f}")
