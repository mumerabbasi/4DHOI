...
def run_openai_image_edit_once(
    ...
    if not data:
        raise RuntimeError("OpenAI image edit response did not contain data.")
    image_b64 = getattr(data[0], "b64_json", None)
    if not image_b64:
        raise RuntimeError(
            "OpenAI image edit response did not contain b64_json image data."
        )
...
def run_openai_image_edit(
    ...
    for attempt_index in range(1, max_attempts + 1):
        ...
        try:
            ...
            print(
                "OpenAI image edit attempt failed: "
                f"{type(exc).__name__}: {exc}"
            )
            if attempt_index >= max_attempts:
                raise
            sleep_s = max(0.0, float(args.image_retry_sleep_s))
            print(f"Retrying OpenAI image edit in {sleep_s:.1f}s...")
            time.sleep(sleep_s)
...
def run_agentic_loop(args: argparse.Namespace, assets: dict[str, Any]) -> int:
    ...
    for round_index in range(1, int(args.max_rounds) + 1):
        ...
        print(
            f"OpenAI image 1: {round_prompt_dir / '01_reference_image.png'}"
        )
        print(
            f"OpenAI image 2: {round_prompt_dir / '02_canvas_image.png'}"
        )
