...
- Hips: required contact with target object 'brown small wooden table'. Use #FF0000 / RGB(255, 0, 0) (pure red). SIG note: the hips rest on the table surface while sitting
- Left Hand: required contact with target object 'brown small wooden table'. Use #00FF00 / RGB(0, 255, 0) (pure green). SIG note: the left hand rests on the table for balance
...
The current masks are too small and thin. Increase the size of the red hips mask to cover the central area of the table surface where the person is sitting. Increase the size of the green left-hand mask and the magenta right-hand mask to cover the areas on the left and right edges of the table surface where the hands are resting.
