...
The red left-hand mask and the green right-hand mask are currently placed in a single vertical stack on the vacuum handle. In the reference image, the hands are positioned side-by-side or overlapping on the handle, not stacked vertically. Move the red left-hand mask to the lower part of the grip and the green right-hand mask to the upper part of the grip so that they correctly cover the two hands as seen in the reference image, ensuring they are properly aligned with the handle's geometry.
