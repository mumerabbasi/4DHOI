...
def load_obj(filename, clear_ks=True, mtl_override=None):
    ...
    for line in lines:
        ...
        if prefix == 'usemtl': # Track used materials
            ...
            vv = vs[0].split('/')
            v0 = int(vv[0]) - 1
            t0 = int(vv[1]) - 1 if len(vv) >= 2 and vv[1] != "" else -1
            n0 = int(vv[2]) - 1 if len(vv) >= 3 and vv[2] != "" else -1
            for i in range(nv - 2): # Triangulate polygons
                vv = vs[i + 1].split('/')
                v1 = int(vv[0]) - 1
                t1 = int(vv[1]) - 1 if len(vv) >= 2 and vv[1] != "" else -1
                n1 = int(vv[2]) - 1 if len(vv) >= 3 and vv[2] != "" else -1
                vv = vs[i + 2].split('/')
                v2 = int(vv[0]) - 1
                t2 = int(vv[1]) - 1 if len(vv) >= 2 and vv[1] != "" else -1
                n2 = int(vv[2]) - 1 if len(vv) >= 3 and vv[2] != "" else -1
...
def write_obj(folder, name, mesh, save_material=True, verbose=False):
    obj_file = os.path.join(folder, f'{name}.obj')
    if verbose:
    ...
    if save_material:
        mtl_file = os.path.join(folder, f'{name}.mtl')
        if verbose:
            print("Writing material: ", mtl_file)
        material.save_mtl(mtl_file, mesh.material, name)
