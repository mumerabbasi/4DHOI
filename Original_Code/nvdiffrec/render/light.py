    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if specular:
            ...
            if not hasattr(self, '_FG_LUT'):
                self._FG_LUT = torch.as_tensor(np.fromfile(osp.join(ROOT_DIR, 'data/irrmaps/bsdf_256_256.bin'), dtype=np.float32).reshape(1, 256, 256, 2), dtype=torch.float32, device='cuda')
