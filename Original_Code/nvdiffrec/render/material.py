...
def load_mtl(fn, clear_ks=True):
    ...
    for line in lines:
        ...
        if 'newmtl' in prefix:
            ...
            elif prefix in ('map_ns', 'map_ke', 'map_d'):
                continue
            else:
...
@torch.no_grad()
def save_mtl(fn, material, tex_prefix='texture'):
    ...
    with open(fn, "w") as f:
        ...
        if material is not None:
            ...
            if 'kd' in material.keys():
                f.write(f'map_Kd {tex_prefix}_kd.png\n')
                texture.save_texture2D(os.path.join(folder, f'{tex_prefix}_kd.png'), texture.rgb_to_srgb(material['kd']))
            if 'ks' in material.keys():
                f.write(f'map_Ks {tex_prefix}_ks.png\n')
                texture.save_texture2D(os.path.join(folder, f'{tex_prefix}_ks.png'), material['ks'])
            if 'normal' in material.keys():
                f.write(f'bump {tex_prefix}_n.png\n')
                texture.save_texture2D(os.path.join(folder, f'{tex_prefix}_n.png'), material['normal'], lambda_fn=lambda x:(util.safe_normalize(x)+1)*0.5)
        else:
            f.write('Kd 1 1 1\n')
            f.write('Ks 0 0 0\n')
            f.write('Ka 0 0 0\n')
...
def merge_materials(materials, texcoords, tfaces, mfaces, max_res=None):
    ...
    for fi in range(len(tfaces)):
        ...
        for vi in range(3):
            ...
            if not (matIdx in new_tverts[ti]): # create new vertex
                # https://stackoverflow.com/a/43805607
                new_tverts_data.append([(matIdx + (texcoords[ti][0]) % 1) / s_coeff[1], (texcoords[ti][1] % 1) / s_coeff[0]]) # Offset texture coodrinate (x direction) by material id & scale to local space. Note, texcoords are (u,v) but texture is stored (w,h) so the indexes swap here
