...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}p{0.16\textwidth}p{0.24\textwidth}X@{}}
    ...
    \(\Pi_{\mathrm{sel}}\) & Best human-frame selection & \path{4DHSI/02_Generate_Human_Frame/prompt_select_best_human_frame.md} \\
    \(\Pi_{\mathrm{con}}\) & Contact overlay generation & \path{4DHSI/03_Estimate_Contact_Agentic/prompt_estimate_contact.md} \\
    \(\Pi_{\mathrm{eval}}\) & Contact VLM evaluation & \path{4DHSI/03_Estimate_Contact_Agentic/prompt_evaluate_contact.md} \\
    \bottomrule
  \end{tabularx}
\end{table}
