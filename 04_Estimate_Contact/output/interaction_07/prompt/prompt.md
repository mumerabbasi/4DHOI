...
Hips: The hips is in contact with the target object. Copy the visible hips mask shape from the Reference Image onto the matching location in the returned overlay image using solid #0000FF / RGB(0, 0, 255) (pure blue).
Left Leg: The left leg is in contact with the target object. Copy the visible left leg mask shape from the Reference Image onto the matching location in the returned overlay image using solid #00FF00 / RGB(0, 255, 0) (pure green).
Right Leg: The right leg is in contact with the target object. Copy the visible right leg mask shape from the Reference Image onto the matching location in the returned overlay image using solid #FF0000 / RGB(255, 0, 0) (pure red).
