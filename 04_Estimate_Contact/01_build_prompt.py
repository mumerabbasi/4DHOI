...
def main(argv: list[str] | None = None) -> Path:
    ...
    scene_image_path = (
        ...
        if args.scene_image
        else selection_json_path.parent / "scene_image.png"
    )
    target_mask_path = (
        Path(args.target_mask).resolve()
        if args.target_mask
        else selection_json_path.parent / "target_mask.png"
    )
    ...
    target_mask = load_binary_mask(target_mask_path, expected_hw=(image_h, image_w))
    target_bbox = bbox_from_mask(target_mask)
    if target_bbox is None:
        raise ValueError(f"Target mask is empty: {target_mask_path}")
    ...
    crop_xyxy = pad_bbox(
        union_bboxes([target_bbox, human_bbox]),
        image_width=image_w,
