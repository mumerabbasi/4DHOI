...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Estimate target-object contact masks with Gemini.",
    )
    parser.add_argument("--video_name", default="video_01")
    parser.add_argument("--prompt", default=None)
    parser.add_argument("--scene-image", default=None)
    parser.add_argument("--target-render", default=None)
    ...
    parser.add_argument("--color-max-distance", type=float, default=120.0)
...
def main(argv: list[str] | None = None) -> Path:
    ...
    target_render_path = (
        Path(args.target_render).resolve()
        if args.target_render
        else output_root / "target_render.png"
    )
    sig_json_path = (
    ...
    contact_overlay_path = output_root / "contact_overlay_target.png"
    ...
    if contact_overlay_path.exists() and not args.overwrite_overlay:
        ...
        if contact_overlay_path.exists():
            print(
                "Overwriting existing contact overlay: "
                f"{contact_overlay_path}"
            )
    ...
    print(f"Wrote contact overlay: {contact_overlay_path}")
