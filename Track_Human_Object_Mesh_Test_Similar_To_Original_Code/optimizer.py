"""Optimization loop for joint human-object mesh refinement."""
...
def _print_optimization_header(
    ...
    print(f"  edges:    {len(context.resolved_edges)}")
    print(
...
def run_joint_optimization(
    ...
    for it in range(args.adam_iters):
        ...
        if it % args.log_interval == 0 or it == args.adam_iters - 1:
            row = build_loss_row(it, result, args)
            ...
            if (
                ...
                for line in format_loss_log(it, args.adam_iters, result, args):
                    print(line)
    ...
    frame_rows = build_frame_loss_rows(0, final_diagnostic, args)
    ...
    if args.optimize_human:
        final_human_verts_np = (
            apply_local_se3_sequence(
                context.human_data.base_verts,
                human_delta_rotvecs.detach(),
                human_delta_trans.detach(),
                context.human_data.centers,
            )
            .cpu()
            .numpy()
            .astype(np.float32)
    ...
    return OptimizationResult(
        ...
        final_human_verts_np=final_human_verts_np,
