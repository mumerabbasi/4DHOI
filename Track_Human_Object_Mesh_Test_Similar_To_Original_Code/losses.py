...
def _sequence_midpoint_diagnostic(
    seq: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    num_frames = seq.shape[0]
    per_frame = torch.zeros(num_frames, device=seq.device, dtype=seq.dtype)
    ...
    midpoint_diff = seq[1:-1] - 0.5 * (seq[:-2] + seq[2:])
...
def _rotation_static_diagnostic(
    ...
    rotvecs = matrix_to_axis_angle(rot_mats)
    return _sequence_static_diagnostic(rotvecs)


def _rotation_midpoint_diagnostic(
    rot_mats: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    rel_rot = torch.matmul(rot_mats[:-2].transpose(1, 2), rot_mats[2:])
    rel_rotvec = matrix_to_axis_angle(rel_rot)
    interp = torch.matmul(
        rot_mats[:-2],
        axis_angle_to_matrix(0.5 * rel_rotvec),
    )
    interp_to_mid = torch.matmul(
        interp.transpose(1, 2),
        rot_mats[1:-1],
    )
    midpoint_vals = matrix_to_axis_angle(interp_to_mid).pow(2).sum(dim=1)
    per_frame[1:-1] = midpoint_vals
    return midpoint_vals.mean(), per_frame


def _compute_object_smoothness_diagnostic(
    trans: torch.Tensor,
    is_translational: bool,
    is_rotational: bool,
    rot_mats: torch.Tensor,
    ...
    num_frames = trans.shape[0]
    per_frame = torch.zeros(num_frames, device=trans.device, dtype=trans.dtype)
    loss = trans.new_tensor(0.0)

    if is_translational:
        trans_loss, trans_frames = _sequence_midpoint_diagnostic(trans)
    else:
        trans_loss, trans_frames = _sequence_static_diagnostic(trans)
        trans_loss = trans_loss * 10.0
        trans_frames = trans_frames * 10.0
    loss = loss + trans_loss
    per_frame = per_frame + trans_frames.to(per_frame.dtype)

    if is_rotational:
        rot_loss, rot_frames = _rotation_midpoint_diagnostic(rot_mats)
    else:
        rot_loss, rot_frames = _rotation_static_diagnostic(rot_mats)
        rot_loss = rot_loss * 10.0
        rot_frames = rot_frames * 10.0
    loss = loss + rot_loss.to(loss.dtype)
    per_frame = per_frame + rot_frames.to(per_frame.dtype)

    return loss, per_frame


def _compute_object_smoothness_loss(
    trans: torch.Tensor,
    is_translational: bool,
    is_rotational: bool,
    rot_mats: torch.Tensor,
) -> torch.Tensor:
    scalar, _ = _compute_object_smoothness_diagnostic(
        trans,
        is_translational,
        is_rotational,
        rot_mats,
...
def penetration_weight_schedule(iteration: int, total_iters: int) -> float:
    if total_iters <= 1:
        return 0.0
    return min(iteration / float(total_iters), 1.0)
...
def compute_all_losses(
    ...
    for slug in obj_keys:
        loss_smooth = loss_smooth + _compute_object_smoothness_loss(
            eff_trans[slug],
...
def compute_final_loss_diagnostics(
    ...
    if obj_keys:
        for slug in obj_keys:
            _, smooth_frames = _compute_object_smoothness_diagnostic(
                eff_trans[slug],
                objects[slug].state.is_translational,
                objects[slug].state.is_rotational,
                eff_rot_mats[slug],
