...
FULL_FRAME_SENSOR_HEIGHT_MM = 24.0
...
def load_focal_length_mm_from_resized_camera(camera_path: Path) -> float:
    ...
    if not isinstance(intrinsics, list) or len(intrinsics) != 3:
        raise ValueError(f"Expected a 3x3 'intrinsics' matrix in {camera_path}.")

    width_px = int(payload["width"])
    height_px = int(payload["height"])
    if width_px <= 0 or height_px <= 0:
        raise ValueError(
            f"Expected positive width/height in {camera_path}, got {(width_px, height_px)}."
        )

    fx_px = float(intrinsics[0][0])
    fy_px = float(intrinsics[1][1])
    focal_mm_x = fx_px * (FULL_FRAME_SENSOR_WIDTH_MM / width_px)
    focal_mm_y = fy_px * (FULL_FRAME_SENSOR_HEIGHT_MM / height_px)

    # resized_camera.json may be produced from a vertically cropped full-frame image,
    # so the horizontal focal length remains the reliable full-frame reference.
    if focal_mm_x <= 0 or focal_mm_y <= 0:
        raise ValueError(
            f"Derived non-positive focal lengths from {camera_path}: "
            f"fx={focal_mm_x}, fy={focal_mm_y}"
        )
    return focal_mm_x
...
def resolve_f_mm(video_dir_name: str, cli_f_mm: int | None | str) -> int | None:
    ...
    return int(round(load_focal_length_mm_from_resized_camera(intrinsics_path)))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "When enabled, run one GVHMR pass per segmented human using a masked video. "
            "Use --no-use_masked_video to skip masking and run once on the unmodified "
            "original video as person_1."
        ),
    )
    parser.add_argument(
        "--f_mm",
        ...
        help=(
            ...
            "Default: derive it from fx in Generate_Video/output/<video_dir>/"
            "resized_camera.json using the 36mm full-frame sensor width, then round to int. "
    ...
    if args.use_masked_video:
        ...
        human_specs = [HumanTrackSpec(name="person_1", masks_dir=None)]
        print(
            f"Masked-video generation disabled for {args.video_name}. "
            "Using the original video as person_1."
        )
