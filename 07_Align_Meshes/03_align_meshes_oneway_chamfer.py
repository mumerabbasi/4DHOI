"""Mesh-to-depth alignment using one-way chamfer losses on all mesh samples.
...
Compared to align_meshes.py, this standalone variant removes the one-time
projection/visibility subset selection and uses all sampled mesh points
...
def build_fixed_visible_subset(
    ...
    del z_abs_tol_m
    del z_rel_tol
    del min_visible_subset_points_per_mesh
    del focal_scale_for_visibility
    ...
    return FixedVisibleSubset(
        ...
        mode="all_samples",
        strict_visible_count=n_samples,
        in_frame_positive_count=n_samples,
        positive_depth_count=n_samples,
    )
...
def save_chamfer_debug_snapshot(
    ...
    cv2.putText(
        right,
        "Projected mesh samples (colored only)",
        (12, 28),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (255, 255, 255),
...
def build_resume_signature(
    *,
    ...
    return {
        "loss_variant": "one_way_2d3d_all_samples",
...
def optimize_scale_txyz_chamfer(
    ...
    if visible_subset_indices.shape[0] == 0:
        msg = (
            "No model sample points available after subset selection "
    ...
    print(
        f"[{mesh_name}] model sample subset: {n_model_visible}/{n_model_total} "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.6,
        help=(
            "Compatibility argument retained from align_meshes.py. "
            "Unused in this all-samples variant."
        ),
    )
...
def main() -> None:
    ...
    summary_out = {
        ...
        "energy_terms": {
            ...
            "model3d": "M3D' = transformed sampled mesh surface points",
