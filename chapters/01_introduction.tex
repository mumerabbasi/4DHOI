...
\begin{enumerate}
  \item \textbf{Agentic contact estimation.} The central contribution is a procedure that localizes precise, per-body-part contact regions on a scene surface and makes them reliable through a closed verify-and-correct loop, in which a vision-language evaluator inspects each generated contact against the intended interaction and issues corrections until the contacts are accepted. Because the regions are localized visually rather than inferred from a named element, and are verified before use, the procedure resolves contact on small and large functional surfaces alike and turns an unreliable single generation into a dependable optimization target.

  \item \textbf{A zero-shot pipeline for scene-grounded interaction synthesis.} The thesis composes pretrained foundation models with explicit camera geometry and a scene-aware optimization into a complete pipeline that synthesizes a static human--scene interaction from a language instruction, a single posed view, and a reconstructed mesh, without any interaction-specific training, lifting the visually localized contacts to the mesh and refining a body to meet them while avoiding penetration.

  \item \textbf{An evaluation methodology for scene-side contact.} Because no contact ground truth exists for these interactions, the thesis introduces a manual scene-side contact annotation that mirrors the method's own contact representation, together with a vision-language rubric evaluation that scores each interaction along target-object, action, contact, and physical-plausibility criteria, making the scene-side quality of a synthesized interaction measurable.
\end{enumerate}
...
\begin{itemize}
  \item \Cref{chap:related-work} reviews prior work on human--scene interaction, tracing how contact has been represented and where existing methods leave the scene side of the contact unresolved, and positions the proposed method against that gap.
  \item \Cref{chap:methodology} develops the method in full, detailing the five-module pipeline, the agentic contact-estimation procedure, and the scene-aware optimization.
  \item \Cref{chap:experiments} presents the evaluation, describing the interactions, metrics, and baseline, and reporting the quantitative and qualitative results together with an analysis of failure cases.
  \item \Cref{chap:conclusion} answers the research questions, summarizes the contributions, and discusses the limitations and directions for future work.
\end{itemize}
