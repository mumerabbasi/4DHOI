...
from first_frame_eval_common import run_resize_first_frame_eval
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Resize a generated first frame to 1280x720 with "
            "geometry-preserving scaling, then run geometry evaluation "
            "and generate a 3D target overlay."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Frame filename or absolute path.",
    )
    parser.add_argument(
        "--generated-root",
        default=None,
    ...
    result = run_resize_first_frame_eval(
    ...
    print(f"Saved resized frame: {outputs['resized_frame_path']}")
    print(f"Saved resized target mask: {outputs['target_mask_path']}")
    print(f"Saved camera JSON: {outputs['camera_json_path']}")
