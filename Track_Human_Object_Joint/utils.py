...
def resolve_dirs(
    ...
    cotracker = (
        Path(args.cotracker_video_dir)
        if args.cotracker_video_dir
        else parent / "Estimate_Optical_Flow" / "output_cotracker" / vname
    )
    seg_obj = (
...
def resolve_frames_dir(dirs: dict[str, Path]) -> Path | None:
    """Return the standard frame directory used in this repo."""
    cotracker_frames = dirs["cotracker"] / "_frames"
    if cotracker_frames.exists():
        return cotracker_frames
    seg_frames = dirs["seg_vid"] / "_frames"
