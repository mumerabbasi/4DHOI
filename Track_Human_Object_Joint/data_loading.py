...
def _sample_mask_bilinear_single(
...
def load_problem_context(
    ...
    for idx, state in enumerate(pag.object_states):
        ...
        track_dir = dirs["cotracker"] / slug
        tracks_path = track_dir / "tracks.npy"
        vis_path = track_dir / "visibility.npy"
        object_mask_dir = (
            dirs["seg_vid"] / "objects" / slug / "object_segmentation" / "masks"
        )

        if not mesh_path.exists():
            print(f"  [SKIP] {slug}: aligned mesh not found at {mesh_path}")
            continue
        if not tracks_path.exists() or not vis_path.exists():
            print(
                f"  [SKIP] {slug}: CoTracker outputs not found at {track_dir}"
            )
            continue
        if not object_mask_dir.exists():
            print(
                f"  [SKIP] {slug}: object masks not found at {object_mask_dir}"
            )
            continue
        ...
        mask_paths = _list_mask_files(object_mask_dir)
        masks_np, mask_h, mask_w = _load_mask_stack(
            mask_paths,
        ...
        tracks_nt2, vis_nt = _normalize_tracks_vis_with_mask_length(
            tracks_raw,
            vis_raw,
            int(masks_np.shape[0]),
        )

        t_total = min(num_frames, masks_np.shape[0], tracks_nt2.shape[1], vis_nt.shape[1])
        if t_total <= 0:
            print(f"  [SKIP] {slug}: no overlapping frames")
            continue

        masks_np = masks_np[:t_total]
        tracks_nt2 = tracks_nt2[:, :t_total].astype(np.float32)
        vis_nt = vis_nt[:, :t_total].astype(np.float32)

        if t_total < num_frames:
            pad_t = num_frames - t_total
            masks_np = np.concatenate(
                [masks_np, np.repeat(masks_np[-1:], pad_t, axis=0)],
                axis=0,
            )
            tracks_nt2 = np.concatenate(
                [tracks_nt2, np.repeat(tracks_nt2[:, -1:, :], pad_t, axis=1)],
                axis=1,
            )
            vis_nt = np.concatenate(
                [vis_nt, np.zeros((vis_nt.shape[0], pad_t), dtype=np.float32)],
                axis=1,
            )
        ...
        valid_seed_mask = mapping.valid_seed_mask
        tracks_valid = tracks_nt2[valid_seed_mask]
        vis_valid = vis_nt[valid_seed_mask]
        valid_track_count = int(tracks_valid.shape[0])
        if valid_track_count < int(args.min_valid_tracks):
            print(
                f"  [SKIP] {slug}: only {valid_track_count} valid CoTracker points "
                f"(need {args.min_valid_tracks})"
            )
            continue

        tracked_poses = np.tile(
            np.eye(4, dtype=np.float32)[None, :, :],
            (num_frames, 1, 1),
        )
        zero_seq = np.zeros((num_frames, 3), dtype=np.float32)
        obs_uv = torch.from_numpy(tracks_valid).float().to(device).permute(1, 0, 2)
        vis_tm = torch.from_numpy(vis_valid).float().to(device).permute(1, 0)
        ...
        objects[slug] = ObjectData(
            ...
            .float()
            .to(device),
            tracked_rotvecs=torch.from_numpy(zero_seq).float().to(device),
            tracked_trans=torch.from_numpy(zero_seq).float().to(device),
            track_points0_cv=mapping.points_cv,
            track_obs_uv=obs_uv,
            track_vis_tm=vis_tm,
            track_masks=masks_t,
            track_seed_count=int(seed_uv.shape[0]),
            track_valid_count=valid_track_count,
            pnp_init_info=[],
        ...
        print(
            ...
            f"parts ({part_names}), "
            f"{valid_track_count}/{seed_uv.shape[0]} valid CoTracker points"
        )
