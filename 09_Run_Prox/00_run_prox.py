#!/usr/bin/env python3
"""Run the PROX baseline on calibrated ScanNet++ interactions."""
...
def human_grid_bounds(vertices_world: Any) -> tuple[Any, Any]:
    import numpy as np
    ...
    bbox_min -= np.asarray(
        [SDF_HORIZONTAL_MARGIN_M, SDF_HORIZONTAL_MARGIN_M, SDF_VERTICAL_MARGIN_M],
        dtype=np.float32,
    )
    bbox_max += np.asarray(
        [SDF_HORIZONTAL_MARGIN_M, SDF_HORIZONTAL_MARGIN_M, SDF_VERTICAL_MARGIN_M],
...
def load_and_crop_scene_mesh(path: Path, bbox_min: Any, bbox_max: Any) -> Any:
    ...
    crop_min = np.asarray(bbox_min, dtype=np.float32) - SCENE_CROP_EXTRA_M
    crop_max = np.asarray(bbox_max, dtype=np.float32) + SCENE_CROP_EXTRA_M
    inside = np.all((vertices >= crop_min) & (vertices <= crop_max), axis=1)
    keep_faces = np.any(inside[faces], axis=1)
    if not np.any(keep_faces):
        raise RuntimeError("The human-centered scene crop contains no mesh faces.")
...
def bbox_corners(bbox_min: Any, bbox_max: Any) -> Any:
    import numpy as np
...
def camera_observes_bbox(camera: Camera, bbox_min: Any, bbox_max: Any, anchor: Any) -> bool:
    import numpy as np

    anchor_camera = world_to_camera(np.asarray(anchor)[None], camera)[0]
    if anchor_camera[2] <= 0.05:
        return False
    anchor_u = (
        camera.intrinsics[0, 0] * anchor_camera[0] / anchor_camera[2]
        + camera.intrinsics[0, 2]
    )
    anchor_v = (
        camera.intrinsics[1, 1] * anchor_camera[1] / anchor_camera[2]
        + camera.intrinsics[1, 2]
    )
    if not (0.0 <= anchor_u < camera.width and 0.0 <= anchor_v < camera.height):
        return False
    points = world_to_camera(bbox_corners(bbox_min, bbox_max), camera)
    valid = points[:, 2] > 0.05
    if not np.any(valid):
        return False
    points = points[valid]
    u = camera.intrinsics[0, 0] * points[:, 0] / points[:, 2] + camera.intrinsics[0, 2]
    v = camera.intrinsics[1, 1] * points[:, 1] / points[:, 2] + camera.intrinsics[1, 2]
    return bool(
        u.max() >= 0.0
        and u.min() < camera.width
        and v.max() >= 0.0
        and v.min() < camera.height
    )


def select_observing_cameras(
    cameras: list[Camera],
    ...
    candidates = [
        camera
        for camera in cameras
        if camera_observes_bbox(camera, bbox_min, bbox_max, anchor)
    ]
    if all(camera.name != source_camera.name for camera in candidates):
        candidates.append(source_camera)
    candidates = sorted(candidates, key=lambda camera: camera.name)
    if len(candidates) <= MAX_TSDF_VIEWS:
        return candidates, len(candidates)

    centers = np.stack(
        [
            -(
                camera.rotation_world_to_camera.T
                @ camera.translation_world_to_camera
            )
            for camera in candidates
        ],
        axis=0,
    )
    directions = centers - np.asarray(anchor, dtype=np.float32)[None]
    ...
    min_angular_distance = 1.0 - directions @ directions[source_index]
    while len(selected_indices) < MAX_TSDF_VIEWS:
        min_angular_distance[selected_indices] = -np.inf
        next_index = int(np.argmax(min_angular_distance))
        selected_indices.append(next_index)
        distance_to_new = 1.0 - directions @ directions[next_index]
        min_angular_distance = np.minimum(min_angular_distance, distance_to_new)
...
def run_interaction(name: str, torch: Any, config: dict[str, Any]) -> None:
    ...
    log(f"\n[*] Running PROX for {name}")
    log("  loading human image and ground-truth ScanNet++ camera")
    ...
    log("  running original PROX mean-pose depth and torso initialization")
    ...
    initial_vertices_camera, initial_joints_camera = body_vertices_and_joints(
    ...
    initial_joints_world = camera_to_world(initial_joints_camera, source_camera)
    anchor_world = initial_joints_world[8]
    bbox_min, bbox_max = human_grid_bounds(initial_vertices_world)
    export_mesh(
    ...
    log("  loading and cropping aligned ScanNet++ mesh")
    scene_mesh = load_and_crop_scene_mesh(inputs.mesh, bbox_min, bbox_max)
    observing, candidate_view_count = select_observing_cameras(
        data["cameras"], source_camera, bbox_min, bbox_max, anchor_world
    )
    log(
        f"  rendering {len(observing)} directionally diverse TSDF view(s) "
        f"from {candidate_view_count} observing ScanNet++ DSLR pose(s)"
    )
    ...
    shoulder_distance = float(
        np.linalg.norm(openpose["keypoints"][2, :2] - openpose["keypoints"][5, :2])
    )
    metadata = {
        ...
        "inputs": {
            ...
            "openpose_json": openpose["json"],
            "input_scene": inputs.input_scene,
            "scene_mesh": inputs.mesh,
            "camera_intrinsics": inputs.transforms,
            "camera_poses": inputs.poses,
        },
        "intrinsics": data["intrinsics"],
        "rotation_world_to_camera": source_camera.rotation_world_to_camera,
        "translation_world_to_camera": source_camera.translation_world_to_camera,
        "camera_to_world": camera_to_world_transform(source_camera),
        "keypoints_format": (
            "native OpenPose BODY_25 + both 21-joint hands + face landmarks 17:68"
        ),
        "openpose_command": openpose["command"],
        "initialization": (
            "upstream PROX zero-VPoser initialization; the same initialization "
            "is run once beforehand only to center the custom TSDF grid"
        ),
        "smplx_gender": config["gender"],
        "optimizer_implementation": PROX_ROOT / "prox" / "fit_single_frame.py",
        "optimizer_entrypoint": "fit_single_frame",
        "shoulder_distance_pixels": shoulder_distance,
        "upstream_side_view_retry_enabled": shoulder_distance < 25.0,
        "contact_body_parts": CONTACT_BODY_PARTS,
        "contact_vertex_count": int(len(load_contact_vertices())),
        "contact_scene_vertex_count": int(len(scene_mesh.vertices)),
        "uses_module03_contact_information": False,
        "module03_role": "interaction eligibility only",
        "sdf": sdf_metadata,
        ...
        "prox_config": PROX_CONFIG,
        "optimization_config": config,
    }
    save_json(final_root / "metadata.json", metadata)
    log(f"  wrote {final_root}")
    del body_model, vposer, pose_embedding, scene_mesh
    gc.collect()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run original PROX losses on calibrated ScanNet++ interactions."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    validate_openpose()
    torch.manual_seed(24017)
    torch.cuda.manual_seed_all(24017)
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    log(f"[*] Selected {len(names)} interaction(s): " + ", ".join(names))
    for name in names:
        run_interaction(name, torch, config)
    log(f"\n[*] PROX finished for {len(names)} interaction(s).")
